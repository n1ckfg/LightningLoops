...
function main() {
   ...
   // ~ ~ ~ ~ ~ ~
    var testCubeGeo = new THREE.BoxGeometry(1, 1, 1);
    var testCube = new THREE.Mesh(testCubeGeo, new THREE.MeshBasicMaterial({ color: 0xff0000 }));
    scene.add(testCube);
    ...
    */
    ...
    function addTempPoints() {

    }

    function createTempStroke() {
    ...
    function updateCubePos() {
        //if (checkVel(velX)) testCube.position.y += -rotScaler(rotX); // TILT: subjective U/D
        //if (checkVel(velY)) testCube.position.x += rotScaler(rotY); // ROLL: subjective L/R
        //if (checkVel(velZ)) testCube.position.z += -rotScaler(rotZ) * 0.5; // PAN: subjective F/B

        //testCube.lookAt(camera);

        testCube.position.x += velZ;
        testCube.position.y += -velX;
        //testCube.position.z += velY;

        //console.log(testCube.position);
        //cubePos = tween(cubePos, targetPos, ease);
        //console.log(cubePos.x + " " + targetPos.x);
        //testCube.position.set(targetPos);        
    }
    ...
    function animate(timestamp) {
        ...
        if (frameDelta >= frameInterval) {
            ...
            for (var i=scene.children.length; i>=0; i--) {
                if (scene.children[i] !== camera && scene.children[i] !== textMesh  && scene.children[i] !== room && scene.children[i] !== testCube) {
        ...
        //ddController.scale.set(1, 1, 1);//0.05, 0.05, 0.05);
        //ddController.position.set(2, 0, 2);
        //console.log(velX);
        updateCubePos();
