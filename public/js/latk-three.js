...
function readLatk(filepath) {
    ...
    */
    loadJSON(filepath, function(response) {
        latkThree = new LatkThree(JSON.parse(response).grease_pencil[0]);

      	animate();
        console.log(latkThree.data);
    });
}
...
function endStroke() {  // TODO draw on new layer
	...
	isDrawing = false;
    var last = latkThree.layers.length-1;
    latkThree.layers[last].frames[layers[last].counter].push(tempStroke);
    //~
...
class LatkThree {
    ...
    constructor(data) {
        ...
        for (var h=0; h<this.data.layers; h++) {
        	var frames = [];
            for (var i=0; i<this.data.layers[0].frames.length; i++) {
                var strokes = [];
                for (var j=0; j<this.data.layers[0].frames[i].strokes.length; j++) {
                    var geometry = new THREE.Geometry();
                    geometry.dynamic = true;
